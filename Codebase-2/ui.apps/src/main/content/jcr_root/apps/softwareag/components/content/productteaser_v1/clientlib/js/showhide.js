...
(function(document, $) {
	...
	function showHide(component, element) {
		...
		var $target = $(target);
		var targetlink = $(document).find("[name='./enableLink']").data("cqDialogCheckboxShowhideTarget");
		var targetbutton = $(document).find("[name='./enableButton']").data("cqDialogCheckboxShowhideTarget");
		...
		if (target) {
			...
			if (component.checked) {
				...
				if (target == ".linkfield") {
					$("input[name='./enableButton']:checkbox").prop('checked', false);
					$(targetbutton).addClass("hide");
				}
				if (target == ".buttonfield") {
					$("input[name='./enableLink']:checkbox").prop('checked', false);
					$(targetlink).addClass("hide");
				}
			}
		}
	}
